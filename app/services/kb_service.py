...
def _query_namespace(query_vector, top_k: int, namespace: str):
    results = index.query(
        vector=query_vector,
        top_k=top_k,
...
def search_kb(query: str, top_k: int = 3, user_id: int = None):
    ...
    matches = _query_namespace(query_vector, top_k, primary_namespace)
...
async def search_kb_async(query: str, top_k: int = 3, user_id: int = 1):
    return await asyncio.to_thread(search_kb, query, top_k, user_id)
